...
import { TableScroll, stickyHead } from "@/components/admin/TableShell";
        ...
        <table className="w-full text-sm">
          <thead className={stickyHead}>
            <tr className="border-b border-black/10 text-left text-xs text-black/50 dark:border-white/10 dark:text-white/50">
              ...
              <SortHeader label="In Progress" sortKey="progress" activeKey={ctl.sortKey} dir={ctl.dir} onSort={ctl.toggleSort} className="text-center" defaultDir="desc" />
              <SortHeader label="Delivered" sortKey="delivered" activeKey={ctl.sortKey} dir={ctl.dir} onSort={ctl.toggleSort} className="text-center" defaultDir="desc" />
              <SortHeader label="Earnings" sortKey="earnings" activeKey={ctl.sortKey} dir={ctl.dir} onSort={ctl.toggleSort} className="text-right" defaultDir="desc" />
            </tr>
          </thead>
          <tbody className="text-black/75 dark:text-white/75">
            {pageItems.map((p) => (
              <tr
                key={p.id}
                className="cursor-pointer border-b border-black/10 last:border-0 transition-colors hover:bg-yellow-400/10 dark:border-white/10"
                onClick={() => setSelected(p)}
              >
                <td className="p-3">
                  <div>
                    <p className="font-semibold text-black dark:text-white">{p.full_name ?? "—"}</p>
                    {p.phone && <p className="text-xs text-black/50 dark:text-stone-500">{p.phone}</p>}
                  </div>
                </td>
                <td className="p-3">
                ...
                <td className="p-3 text-center">
                  {p.inProgressCount > 0 ? (
                    ...
                    <span className="text-black/40 dark:text-stone-600">0</span>
                  )}
                </td>
                <td className="p-3 text-center text-black dark:text-white">{p.deliveredCount}</td>
                <td className="p-3 text-right font-semibold text-lime-600 dark:text-lime-400">
